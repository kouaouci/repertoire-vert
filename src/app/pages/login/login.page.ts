...
import { AlertController, LoadingController, MenuController, NavController, NavParams, ToastController } from '@ionic/angular';
...
export class LoginPage implements OnInit {
  ...
  checkCredentials(password: string) {
    this.authService.verifyAuth(this.user.email, password).then(result => {

      // Result contains repertoire vert id, or -1 if failed auth
      if (result !== -1) { // Auth successful
  ...
  register() {
    ...
    // Start registration
    this.authService.register(this.user.username, this.user.email,
      this.user.password, this.user.password).subscribe(response => {
  ...
  registerToRepVertDB() {
    ...
    this.authService.registerToRepVertDB(this.user).subscribe(response => {
      // Stop loading spinner
      this.isLoading = false;
