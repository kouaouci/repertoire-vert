...
export class AuthService {
  ...
  verifyAuth(email: string, password: string): Promise<number> {
    ...
    return new Promise<number>((resolve, reject) => {
      ...
      this.http.post<{ id: number, message: string }>
        (this.LOGIN_API_URL + "login", authData, { headers: this.headers })
        .subscribe(
          response => {
  ...
  loginUser(id: number, username: string, firstname: string, lastname: string,
    token: string, repVertId: string): void {
    this.isAuthenticated = true;
  ...
  verifyToken(id: number, token: string): Observable<any> {
    return this.http.post<{ code: number, message: string }>(this.REP_VERT_API + "users/" + id + "/token", { token: token });
  }
